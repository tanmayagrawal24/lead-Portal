...
#: response is to look at the reservation and raise the cap for that run.
...
class RunCeilingExceeded(RuntimeError):
    """This single run's reservations exceed §7 control 3's per-run ceiling.
...
def charge_run(
    ...
    *,
    ...
    ceiling_usd: float = RUN_CEILING_USD,
) -> float:
...
def reconcile_run(conn: sqlite3.Connection, *, run_id: int, delta_usd: float) -> None:
    ...
    **This deliberately does not consult control 3, and the omission is the
    ruling.** A reconciliation is not a request to spend — the money is already
    gone, and this is the correction that makes the ledger true. Refusing it
    because the corrected total lands above the per-run ceiling would leave
    `run.est_cost_usd` holding a number known to be wrong, which makes control
    2 — the guard that actually bounds spend — read a falsehood. **A ceiling
    that can block its own bookkeeping is a ceiling that degrades the ledger it
    exists to protect.**

    Separate from `charge_run` for the same reason `_charge_run` is separate
    from `_write_batch_row`: *reserve* and *reconcile* are different operations
    with different rules, and one function taking a flag would be one place for
    a later edit to conflate them. The delta may be negative — B3.1 sends a
    batch's actual back to the submitting run, and an over-estimate corrects
    downward.
    """
    conn.execute(
        "UPDATE run SET est_cost_usd = COALESCE(est_cost_usd, 0) + ? WHERE id = ?",
