...
> - **Tier 2 — first article in index document order.** *Proposed for the record and not recommended for adoption; see §4.*
> - **Tier 3 — shallowest under the index path.** A6 as ratified, unchanged, as the terminal tier.
>
...
| shop | Tier 1 — newest lastmod | Tier 3 — A6 today | differs |
|---|---|---|---|
...
**Tier 1 resolves on all 7**, including both shops the M1.32 guard currently silences. **Tier 3 answers for none of them** — the same shape M1.24 found, where the fallback had been doing the entire job.
...
## 4. Tier 2, and why the corpus argues against adopting it

Tier 2 rests on a convention: *blog indexes render newest-first.* The brief asked for it to be stated as unobserved unless the corpus confirms it. The corpus was checked, and gave a split answer.
...
**The other four indexes carry no date at all**, so the convention is unobserved on `bio-fleischer-laden.de`, `doonails.de`, `smile-store.de` and `snocks.com`. That is the wrong four to be missing: an index that dates its posts already yields `content.blog_last_post` from the index, and M1.32 marks its basis `both`. **The convention is confirmed exactly where it is not needed and untested on both shops where it is** — including the two the guard silences.
...
They pass A6's filter — they are under the index path — and they are listings. Tier 2 would hand a listing page to an Article parser: **M1.16's error, a third time, in the tier introduced to be the safe one.** Navigation precedes content in document order on essentially every template, so this is a structural property of the tier, not a quirk of one shop.

**Recommendation: adopt Tiers 1 and 3; do not adopt Tier 2 now.** It is unverified on the shops that need it, it selects navigation on the one shop where that could be checked, and — decisively — **it would never fire on this corpus**, because Tier 1 resolves all seven. Adding an unobserved convention to a hierarchy in a position where it has never once executed is the M1.4 error with a different string. This project has taken that error at least three times already — `/p/` as a product prefix (M1.4), `jtl-shop` as a JTL signature (M1.9), "shallowest under the blog path is a post" (M1.16) — and each time the convention was plausible, widespread in the wild, and wrong here.

It is written up rather than dropped because there is a real case for it: a shop whose blog index is fetchable but whose shard is missing or unreadable. `zecplus.de` and `lampenflut.de` are that shape for other signals (§10.1). **Add Tier 2 when a shop is observed in that state, with a guard requiring the candidate to be dated by the index itself** — which is what would have excluded `smile-store.de`'s categories.
