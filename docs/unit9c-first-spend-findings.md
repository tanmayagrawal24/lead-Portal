...
**M1.112**, and it is a finding about a money decision, not about a credential.
...
> **UNIT 11 NOTE — read this first (2026-09-04).** This is Unit 9c's report,
> written on 2026-08-21 on `claude/unit9c-first-spend` (PR #7) and reproduced
> here unaltered except for its amendment numbers. **It is a record of what was
> true on 21 August, not a statement about the tree today.** PR #7 never merged;
> a parallel line of work (PR #8, then PR #9) took the numbers M1.101–M1.108
> while it sat open, so every citation below has been renumbered —
> **M1.101→M1.109, M1.102→M1.110, M1.104→M1.111, M1.105→M1.112** — and #7's
> M1.103 is **not carried as a row at all**: that number now belongs to M5's
> `max_uses` ruling, and #7's M1.103 was an update to M1.75 (*"nothing is
> changed"*) rather than a new decision, so its four citations here point at
> M1.75, the row that owns the finding. **M1.113 records the collision itself.**
> The whole account is in `docs/unit11-reconcile-9c-findings.md`; §11 below
> carries a Unit 11 note saying which of its open rows have since closed.
...
**M1.109–M1.112** — *renumbered by Unit 11; see the note above.*
...
## 6. §7 control 3 — built, and proved to refuse (M1.109, M1.110)
...
### 6.4 The finding the build produced (M1.110)
...
| `test_reconciliation_is_never_refused_by_the_per_run_ceiling` | M1.110's ruling, both directions of delta |
...
one API key of writing nine of them. See M1.111.

---

## 9. Still open

- **Was a batch ever submitted? (§10.7b, M1.100, M1.112.)** Still OPEN, and the
...
- **M1.75's recurring cost** — every crawl of a shop serving identical
  robots.txt from apex and `www.` loses that shop to `robots_unavailable`.
- **M1.111** — no erasure path. M7.
...
M1.112 corrects. The listing printed nothing **because it never ran**, and the
...
| §7 control 3 | Specified, not implemented | **BUILT** — `RUN_CEILING_USD`, `charge_run`, 7 tests | M1.109 (§6) |
| `run.est_cost_usd`'s second writer | Unstated | **RULED** — reconciliation is never refused by control 3 | M1.110, `ledger.reconcile_run` (§6.4) |
| §8 erasure path | Not previously registered | **OPEN, with the manual procedure written out** | M1.111 (§8) |
| §10.7b's closing procedure | Specified; never executed | **EXECUTED, failed silently, HARDENED** — terminal verdict line required | M1.112 (§14.2) |
...
(§14.5). **Next free amendment: `M1.<122>`** (angle brackets per M1.94, so this
line is not a citation).

> **UNIT 11 NOTE (2026-09-04) — what has closed since this table was written.**
> The rows above are 21 August's readings and several were overtaken by PR #8
> (M5 completion) and PR #9 (Unit 10) while PR #7 sat open. Specifically:
> **`portal/pagespeed.py` is built** and `run.pagespeed_calls` has a writer
> (migration 016); **`llm_batch.status = 'balance_exhausted'` has its writer**;
> **next free migration is `018`, not `016`** — 016 and 017 were both taken.
...
which is the mechanism rather than the assertion. Re-run after §14 and M1.112
...
robots.txt, so M1.75's collapse cannot occur there. **That is worth stating
plainly — the fixture corpus cannot reproduce M1.75, so CI is not evidence
...
conclusion, and M1.112 is the reason why**: the decisive measurement is the one
...
### 14.2 §10.7b — run first, as instructed, and it did not close (M1.112)
...
That instrument is now hardened (M1.112): the client is constructed on its own
...
writer — precisely M1.45(c), the defect M1.111 recorded one section above.
...
M1.59's distinction and, this unit having spent a section on it, M1.112's.
...
arrived with the crawl, not with the extraction. M1.111 recorded the erasure gap
...
robots.txt and M1.75's collapse cannot occur there.
...
Measured per M1.112, in the environment of the process that would make the call:
...
how it says *forbidden*; this one does. That is M1.112's rule applied to a second
...
> sessions were unable to take. **Next free amendment is now `M1.<122>`**
