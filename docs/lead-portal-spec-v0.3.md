...
|---|---|---|---|---|
| 1 | `schema.product_present`, absent a product page fetched with HTTP 200 (A5.5, A5.6) | A `0` would be a fact about a page we never retrieved | `opp.no_product_schema` (+10) | **nothing — see below** |
| 2 | `content.blog_last_post`, unparseable (§6.2) | An undated index is an unknown, not a stale blog | the whole blog ladder below `no_blog` | `blog_date_unparseable` |
...
**The point of naming it is that the fifth instance is applied rather than argued.** The recurring shape is a rule that awards points for an *absence* — no schema, no posts, no catalogue, no recent post — reading a failure to measure as the absence it rewards. Every such rule is a candidate, and the test is one question: *if this signal is missing or weak, does the rule award points?* If yes, A7 applies.

**Naming it also immediately found a gap.** Instance 1 satisfies parts 1 and 2 and **not part 3**: where no product page is fetched, `schema.product_present` is correctly left unwritten and correctly fires nothing — and no flag is raised, so nobody is told. `catalog_not_measurable` covers the neighbouring case (no product *identifiable*) but not this one (a product identified, the fetch failed), and the two are different: the second is a transient the next run may fix, which is an argument for a different resolution policy, not for silence. **Recorded, not silently fixed**, because adding a fifth review reason is a §6.4 ratification and this note is a naming exercise. It is the first thing to check when A5.5 is next opened.
...
**Those instruments have since been assessed — see `docs/m114-blog-detection-read.md`.** Anchor text, taken with the href wherever it points, reaches **both** unreachable shapes on homepages already stored (`zecplus.de`'s *"Blog"* → `blog.zecplus.de`; `lampenflut.de`'s *"Licht-Ratgeber"* → a root-level slug), with no false positive across the other four shops carrying `blog_exists = 0`. `Article` JSON-LD on the homepage reaches the root-slug shape only. A fourth candidate — feed autodiscovery — is assessed and **rejected**: it fires on 4 of the 6, and on every one it is a platform default (`/rss.xml` on JTL, `/feed/` on WordPress) belonging to a shop with no blog, which is §10.4's rule about platform-shipped strings in a new place. The read also argues that no instrument closes this blocker on its own, because `opp.no_blog` awards points for an absence and therefore falls under **A7** (§5).
