...
| `llm_batch.status = 'balance_exhausted'` | **not in the schema yet** — still true, but the migration number in this row was stale: `010` went to `score.evaluated_on` (M1.74) and the lost stash's `010` is unrecoverable. It ships with **9b's** reservation caller, per M1.45(c), on `014` or later | §7 control 11 |
...
| `run.est_cost_usd` as the §7 control 2 ledger | **read-live, written-by-nobody** — `portal/ledger.py` queries it and every paid surface is gated on it; the column has **no writer** until M5 builds controls 3 and 4, so the ledger reads `$0.00` in production today. Registered deliberately, per M1.45(c): the reader shipping first is the point (M1.69–M1.72) | §7 controls 2–4 |
...
- **The interrupted M5 work is GONE and cannot be recovered.** It was `git stash` entry `interrupted-M5-remnant`, taken 2026-08-17 at `6a5e266`, and a stash is local to a working copy — the container that held it no longer exists, and nothing was ever committed. This bullet read *"stashed, not deleted … worth salvaging"* through Units 6 and 7, which is false and reads as though a `git stash apply` would retrieve it. **M5 is a rebuild from the description below, not a restore.** The surviving artifact is the inventory in the Unit 5 report: `phase2_input_settled` on `Rule` with `assert_declared` enforcing it, the `settled` term that lets §5.4's gate *tighten* rather than only loosen, three-state `_own_brand` and `_owner_operated` abstentions, and untracked `portal/pagespeed.py`, `portal/verify.py` and `migrations/010_phase2_writers.sql`. **Migration 010 is why it could not simply be committed**: it creates `llm_batch_request`, which has **no writer in `portal/` and no registration in this spec** — verified by grep, not assumed — which is M1.45(c)'s shape and exactly what 010's own header comment says a migration must not do. It is also what failed `test_schema.py::test_every_spec_table_exists`. Rebuild it in M5 with its writer, or register it in §10.6 as ahead-of-writer deliberately. The warning against letting it "arrive as a side effect of unstashing" is now moot: there is nothing to unstash.
